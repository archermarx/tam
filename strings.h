...
// sb_join(const char** elems, usize num_elems, char* pat);
//
 ...
 */
char tam_sl_idx(tam_slice_t s, i64 i);
 ...
 */
tam_slice_t tam_reslice(tam_slice_t s, i64 i, i64 j);

/*
 * Construct a subslice using the implicit indices [0, i)
 * Equivalent to s[:i] in python
 */
tam_slice_t tam_slice_prefix(tam_slice_t s, i64 i);

/*
 * Construct a subslice using the implicit indices [i, len(s))
 * Equivalent to s[i:] in python
 */
tam_slice_t tam_slice_suffix(tam_slice_t s, i64 i);
 ...
 */
i64 tam_sl_lstrip(tam_slice_t *s);
 ...
 */
i64 tam_sl_rstrip(tam_slice_t *s);
 ...
 */
i64 tam_sl_strip(tam_slice_t *s);
 ...
 */
i64 tam_sl_cspan(tam_slice_t s, const char *reject);
 ...
 */
i64 tam_sl_span(tam_slice_t s, const char *accept);
 ...
 */
i64 tam_sl_find(tam_slice_t haystack, tam_slice_t needle);
 ...
 */
i64 tam_get_slice_index(i64 i, i64 len) {
  i64 j = i >= 0 ? i : len + i;
  assert(0 <= j && j <= len);
  return j;
}

char tam_sl_idx(tam_slice_t s, i64 i) { return s.buf[tam_get_slice_index(i, s.len)]; }
...
tam_slice_t tam_slice_prefix(tam_slice_t s, i64 i) { return tam_slice_n(s.buf, tam_get_slice_index(i, s.len)); }

tam_slice_t tam_slice_suffix(tam_slice_t s, i64 i) {
  i64 j = tam_get_slice_index(i, s.len);
...
tam_slice_t tam_slice_tok(tam_slice_t *s, const char *delimiters) {
  i64 prefix_size = tam_sl_cspan(*s, delimiters);
  ...
  i64 dlm_size = tam_sl_span(suffix, delimiters);
...
int tam_test_slices() {
  ...
  }
  {
    ...
    i64 trailing = tam_sl_rstrip(&sl3);
    assert(trailing == 24);
...
int tam_test_stringbuilders() {
  ...
  assert((i64)sb.len == 13 + next.len);
  assert((i64)sb.cap == 13 + next.len + 1);
